...
<template>
    <div v-if="props.lesson.type === 'QUIZ'">
        ...
        <div class="flex flex-col items-center my-5">
            <div>
                <p v-if="isFinished" class="text-gray-500">Lesson completed! You've earned <span class="text-primary">{{ props.lesson.content.rewardXp }} XP!</span></p>
            ...
            <div v-if="!isFinished">
                <UButton v-for="option in props.lesson.content.options" :key="option.text" class="m-2" @click="checkQuizAnswer(option.isCorrect)">
                    {{ option.text }}
                </UButton>
            </div>
            <UButton @click="goToNextLesson">{{ props.lesson.nextLessonId ? 'Next Lesson' : 'Back to Modules' }}</UButton>
        </div>
    </div>
</template>